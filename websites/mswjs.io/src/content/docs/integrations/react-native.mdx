...
### Expo Router navigation is broken

**Reason:** MSW's `msw/native` uses `XMLHttpRequestInterceptor`, which globally patches the `XMLHttpRequest` class at the JavaScript runtime level. Expo Router relies on `XMLHttpRequest` or `fetch` internally for navigation mechanics (e.g. loading route metadata, async route resolution). This global patching can interfere with those internal requests even when `onUnhandledRequest: 'bypass'` is set, because the patching itself—not the mocking—is what causes the interference.

**Solution:** Scope your request handlers strictly to specific external API origins and avoid catch-all URL patterns. This prevents MSW from matching (and potentially disrupting) Expo Router's internal requests.
...
export const handlers = [
  // Only intercept specific external API domains.
  http.get('https://api.example.com/*', () => { /* ... */ }),
  http.post('https://api.example.com/*', () => { /* ... */ }),
]
```

If scoping the handlers does not resolve the issue, initialize MSW before Expo Router's navigation tree mounts. Use a `useState` flag to hold off rendering `<Stack />` until MSW is ready, so none of Expo Router's internal requests escape before the interceptor is in place:
...
export default function RootLayout() {
  const [isMockingEnabled, setIsMockingEnabled] = useState(!__DEV__)

  useEffect(() => {
    enableMocking().then(() => setIsMockingEnabled(true))
  }, [])
...
> By returning `null` until `isMockingEnabled` is `true`, Expo Router's navigation tree — and all its internal `XMLHttpRequest` calls — only start after MSW is fully set up.
