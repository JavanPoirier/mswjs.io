...
<Warning>
  MSW patches `XMLHttpRequest` in React Native. If you use Expo Router (or any
  runtime that relies on internal XHR-based navigation), register the root
  component synchronously and start MSW in the background instead of awaiting
  `enableMocking()`.
</Warning>
