...
**Reason:** MSW's `msw/native` uses `XMLHttpRequestInterceptor`, which replaces the methods on the global `XMLHttpRequest.prototype` (e.g. `open`, `send`) at the JavaScript runtime level. Every `XMLHttpRequest` call in the app — including Expo Router's internal navigation requests — goes through the patched prototype regardless of whether any handler matches. Scoping handlers to specific origins does not help, because the interference is caused by the patching itself, not by request matching.

**Solution:** Initialize MSW before Expo Router's navigation tree mounts. Use a `useState` flag to hold off rendering `<Stack />` until MSW is ready, so none of Expo Router's internal `XMLHttpRequest` calls occur before the patched prototype is in its final, stable state:
...
> By returning `null` until `isMockingEnabled` is `true`, Expo Router's navigation tree — and all its internal `XMLHttpRequest` calls — only start after MSW has finished patching `XMLHttpRequest.prototype`.
