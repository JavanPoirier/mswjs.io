...
MSW relies on standard JavaScript classes that are not present in React Native, like `URL` and `MessageEvent`. Please install the polyfills below to guarantee proper MSW execution in React Native.
...
> If your React Native environment does not provide globals like `MessageEvent`, `Event`, `EventTarget`, or `BroadcastChannel`, you may need to add manual shims for them. Make sure that the `Event` polyfill includes standard methods like `stopPropagation()` and `stopImmediatePropagation()`, as MSW relies on them internally.
...
### TypeError: new Proxy target must be an Object

**Reason:** Starting from MSW 2.13.0, the library uses the `rettime` event emitter internally. Versions of `rettime` prior to 0.11.0 create a `Proxy` on `event.stopPropagation`, which crashes on React Native (Hermes) if the `Event` polyfill does not define `stopPropagation` as a function.
...
**Solution:** Ensure that your `Event` polyfill includes a `stopPropagation` method. Add the following to your `msw.polyfills.js` file _before_ the other polyfills:
...
Alternatively, you can force a newer version of `rettime` that does not use `Proxy` for event handling. Add a `resolutions` (Yarn) or `overrides` (npm) field in your `package.json`:
